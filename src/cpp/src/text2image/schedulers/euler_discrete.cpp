...
EulerDiscreteScheduler::EulerDiscreteScheduler(const Config& scheduler_config) : m_config(scheduler_config) {
    ...
    // TODO:
    // if (m_config.timestep_type == TimestepType::CONTINUOUS && m_config.prediction_type ==
...
void EulerDiscreteScheduler::set_timesteps(size_t num_inference_steps) {
    ...
    for (const float& i : m_alphas_cumprod) {
        ...
        sigmas.push_back(std::log(sigma));
    }
    ...
    case InterpolationType::LINEAR: {
        ...
        std::iota(x_data_points.begin(), x_data_points.end(), 1);
